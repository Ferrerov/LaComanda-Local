...
class GestorConsultas
{
    ...
    {
        $objetoAccesoDato = AccesoDatos::dameUnObjetoAcceso();
        $consulta = $objetoAccesoDato->RetornarConsulta("SELECT p.idPedido, p.idMesa, p.idEmpleado, p.idProducto, p.estado, p.precioVenta, p.tiempoEstimado, p.tiempoInicio, p.tiempoFin FROM pedidos p JOIN productos ON p.idProducto = productos.idProducto JOIN empleados ON empleados.idSector = productos.idSector WHERE empleados.idUsuario = '$idUsuario'");
        $consulta->execute();
        $pedidosBuscados = $consulta->fetchAll(PDO::FETCH_CLASS,'pedido');
        return $pedidosBuscados;
    }
    ...
    public static function ActualizarUnPedido($pedido)
    {
        ...
        idMesa = '$pedido->idMesa', idProducto = '$pedido->idProducto', estado = '$pedido->estado', precioVenta = '$pedido->precioVenta', tiempoEstimado = '$pedido->tiempoEstimado', tiempoInicio = '$pedido->tiempoInicio', tiempoFin = '$pedido->tiempoFin'
